...
static constexpr std::array<std::optional<float>, 2> scaling_values{ 0.5f, 2.0f };
...
struct generate_volk_test_scaling_name : generate_volk_test_name {
    ...
    std::string operator()(const ::testing::TestParamInfo<ParamType>& info) const
    {
        ...
        std::string scale_name = fmt::format("{:.2f}", std::get<2>(info.param).value());
...
struct Volk8iConvert16i : KernelSpec<int8_t, int16_t> {
    ...
    static output_type expected(input_type value)
    {
        return static_cast<int16_t>(value) * KERNEL_SCALING_VALUE;
    }
    static void run(output_type* out, const input_type* in, size_t n, const char* impl)
    {
        volk_8i_convert_16i_manual(out, in, n, impl);
    }
...
struct Volk16icConvert32fc : KernelSpec<lv_16sc_t, lv_32fc_t> {
    ...
    static void run(output_type* out, const input_type* in, size_t n, const char* impl)
    {
        volk_16ic_convert_32fc_manual(out, in, n, impl);
    }
...
struct Volk32fConvert64f : KernelSpec<float, double> {
    static output_type expected(input_type value) { return value; }
    static void run(output_type* out, const input_type* in, size_t n, const char* impl)
    {
        volk_32f_convert_64f_manual(out, in, n, impl);
    }
...
struct Volk64fConvert32f : KernelSpec<double, float> {
    static output_type expected(input_type value) { return static_cast<float>(value); }
    static void run(output_type* out, const input_type* in, size_t n, const char* impl)
    {
        volk_64f_convert_32f_manual(out, in, n, impl);
    }
...
struct Volk32fS32fConvert16i : KernelSpec<float, int16_t> {
    ...
    static output_type expected(input_type value, std::optional<float> scale)
    {
        return rounded_saturated<int16_t>(value * *scale);
    }
    static void run(output_type* out,
                    const input_type* in,
                    size_t n,
                    const char* impl,
                    std::optional<float> scale)
    {
        volk_32f_s32f_convert_16i_manual(out, in, *scale, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk32fS32fConvert32i : KernelSpec<float, int32_t> {
    ...
    static void run(output_type* out,
                    ...
                    std::optional<float> scale)
    {
        volk_32f_s32f_convert_32i_manual(out, in, *scale, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk32fS32fConvert8i : KernelSpec<float, int8_t> {
    ...
    static void run(output_type* out,
                    ...
                    std::optional<float> scale)
    {
        volk_32f_s32f_convert_8i_manual(out, in, *scale, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk32iS32fConvert32f : KernelSpec<int32_t, float> {
    ...
    static void run(output_type* out,
                    ...
                    std::optional<float> scale)
    {
        volk_32i_s32f_convert_32f_manual(out, in, *scale, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk8iS32fConvert32f : KernelSpec<int8_t, float> {
    ...
    static void run(output_type* out,
                    ...
                    std::optional<float> scale)
    {
        volk_8i_s32f_convert_32f_manual(out, in, *scale, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk32fcConvert16ic : KernelSpec<lv_32fc_t, lv_16sc_t> {
    ...
    static void run(output_type* out, const input_type* in, size_t n, const char* impl)
    {
        volk_32fc_convert_16ic_manual(out, in, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk16iS32fConvert32f : KernelSpec<int16_t, float> {
    ...
    static void run(output_type* out,
                    ...
                    std::optional<float> scale)
    {
        volk_16i_s32f_convert_32f_manual(out, in, *scale, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct Volk32fS32fX2Convert8u : KernelSpec<float, uint8_t> {
    ...
    static output_type expected(input_type value, std::optional<float> scale, float bias)
    {
    ...
    static void run(output_type* out,
                    ...
                    float bias)
    {
        volk_32f_s32f_x2_convert_8u_manual(out, in, *scale, bias, n, impl);
    }
    static volk_func_desc_t descriptor()
    {
...
struct ConvertTestFunctions<2, Kernel, Parameter> {
    ...
    static void run(typename Kernel::output_type* out,
                    const typename Kernel::input_type* in,
                    const size_t size,
                    const std::string_view implementation,
                    const Parameter&)
    {
        Kernel::run(out, in, size, implementation.data());
    }
};
...
struct ConvertTestFunctions<3, Kernel, Parameter> {
    ...
    static void run(typename Kernel::output_type* out,
                    ...
                    const Parameter& parameter)
    {
        Kernel::run(out, in, size, implementation.data(), std::get<2>(parameter));
    }
};
...
protected:
    ...
    virtual void run_kernel(const std::span<output_type> out,
                            const std::span<const input_type> in,
                            const std::string_view implementation)
    {
        static_assert(std::tuple_size_v<parameter_type> >= 2);
        ConvertTestFunctions<std::tuple_size_v<parameter_type>, Kernel, parameter_type>::
            run(out.data(), in.data(), in.size(), implementation, this->GetParam());
    }
};

template <typename Kernel,
          typename Parameter = std::tuple<std::string, size_t, std::optional<float>>>
...
protected:
    ...
    void run_kernel(const std::span<output_type> out,
                    const std::span<const input_type> in,
...
template <typename Kernel,
          typename Parameter =
              std::tuple<std::string, size_t, std::optional<float>, float>>

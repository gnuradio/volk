                                                  ...
                                                  unsigned int num_points)
{
    ...
    for (; number < eighthPoints; number++) {
        ...
        // Store to buffer and use rintf for round-to-nearest-even
        // (ARMv7 NEON lacks vcvtnq_s32_f32, so use scalar rintf)
        float buf[8];
        vst1q_f32(buf, ret1);
        vst1q_f32(buf + 4, ret2);

        *outputVectorPtr++ = (int16_t)rintf(buf[0]);
        *outputVectorPtr++ = (int16_t)rintf(buf[1]);
        *outputVectorPtr++ = (int16_t)rintf(buf[2]);
        *outputVectorPtr++ = (int16_t)rintf(buf[3]);
        *outputVectorPtr++ = (int16_t)rintf(buf[4]);
        *outputVectorPtr++ = (int16_t)rintf(buf[5]);
        *outputVectorPtr++ = (int16_t)rintf(buf[6]);
        *outputVectorPtr++ = (int16_t)rintf(buf[7]);
    }
